...
| # | Item | Owner | State |
|---|---|---|---|
| 0 | **Marketing audience (PR #58)** — CSV import + sealed lists + compliant broadcasts + /u unsub + Resend webhook + sending domains. Fable-audited 2026-07-18, review fixes committed (7b216ed). Merge on green CI, then server deploy + env (`RESEND_FROM_MARKETING` subdomain split, `RESEND_WEBHOOK_SECRET`, `PUBLIC_API_BASE`). Open decisions in the PR body: abuse gate (charges_enabled) BEFORE public launch; operational-vs-marketing suppression; resubscribe flow; CAN-SPAM postal address; >1000-recipient batching; list blobs ride the test batch → fold into #45 | **Fable** | PR open, CI pending |
