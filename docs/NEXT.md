...
Last updated: 2026-07-27 (pricing + email plan landed; payout liability found).
...
| # | Item | Owner | State |
|---|---|---|---|
| **00** | ✅ **BUILT 2026-07-27 — manual payouts + post-event release shipped.** `interval:"manual"` at account creation + self-healing on `account.updated` + per-sale hold ledger + hourly release sweep + `GET /api/stripe/payouts` + audit script + 25 tests (server 66/66). **Mechanism/ops/limits: `docs/PAYOUTS.md`. Decisions: `PRICING_AND_EMAIL.md` §17.** **BLOCKER RESOLVED → Managed Risk:** Stripe confirmed `application_fee_amount` survives it, so per §16 reconfigure BEFORE onboarding real organisers (record it as a support read, not a documented guarantee — get it restated before the migration lands). **TWO NEW HARD LIMITS, both verified in Stripe's docs:** (1) funds **cannot be held beyond 90 days** (UK; TH 10d, US 2y) measured **per charge, not per event** — so early-bird/festival money is released BEFORE the event, which makes Managed Risk *more* valuable, not less, since liability is then the only control left; (2) `interval:"manual"` is **not a lock** — an Express organiser can still pay themselves from their dashboard; full lockout needs a support request. **User → send on the live Stripe thread:** grant the payout restriction; confirm Managed Risk preserves our payout-schedule control (their doc conditions it on who carries dispute liability); confirm whether existing accounts migrate. **DEPLOY NOTE:** run `npx tsx scripts/payout-schedule-audit.ts --fix` after deploying — existing accounts are still on Stripe's automatic schedule. Legal drafts updated to both limits (not escrow, hold not guaranteed): `TERMS_OF_SERVICE.md` §4, `ORGANISER_TERMS.md` §6, `DATA_INVENTORY.md` §5.2, `PRIVACY_POLICY.md` §10. | **Fable** (money path) | ✅ Built + tested; Managed Risk reconfiguration + 3 Stripe asks outstanding |
| ~~00~~ | (was) 🚨 **DELAYED PAYOUTS — launch blocker for paid ticketing.** `routes/stripe.ts:66` creates accounts with `type: "express"` and no `controller` block ⇒ `controller.losses.payments = "application"` ⇒ **WoCo carries unrecoverable negative balances.** Stripe pays organisers out on the default fast schedule, so a cancelled 500×£25 event puts ~£12.5k on our platform reserve (Stripe freezes it immediately, consumes it after 180 days negative). Build: `delay_days_override` (max 31, verified) + `interval: "manual"` with a post-event release job beyond that. Also pursue **Managed Risk** for NEW accounts (Stripe assumes the liability; needs direct charges — we have them — plus `card_payment` capability and `full` service agreement; existing Express accounts **cannot** be migrated, so cost grows per signup). Spec: `docs/PRICING_AND_EMAIL.md` §7 + **§15 (Stripe human support, authoritative — read this first)**. **RESOLVED:** we ARE on "you handle pricing" and pay **£2/monthly active account + 0.25% + 10p per payout** — the 0.25% is on payout *volume* and takes ~20% of gross ticketing revenue (effective take ~1.20%, not 1.50%). `interval: "manual"` + post-event release is **Stripe's own recommended pattern** for future-delivery businesses ⇒ build that, not `delay_days`. **STILL OPEN — do not guess:** (a) whether `application_fee_amount` survives under Managed Risk (if not, Managed Risk can't carry our 1.5% and Option B is dead — worth a call, not a chat); (b) any max hold period with manual payouts (bot said 90 days, human didn't repeat it — keep the ceiling a constant). **NEW:** Stripe may impose a launch reserve on our platform account (% of volume, rolling 60–90 days, set at underwriting) — working-capital item. **§16 REVERSES the earlier "launch on Option A" call:** resolve the Connect config BEFORE onboarding real organisers — pre-launch with ~no live accounts is the cheapest this decision will ever be, and every organiser onboarded first becomes permanent legacy on worse terms. Build manual payouts meanwhile (required under BOTH branches, so never wasted). | **Fable** (money path) | Fees + payout pattern settled; Managed Risk fork BLOCKING launch |
