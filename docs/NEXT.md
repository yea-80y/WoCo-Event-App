...
| # | Item | Owner | State |
|---|---|---|---|
| 0 | **Marketing audience (PR #58)** — CSV import + sealed lists + compliant broadcasts + /u unsub + Resend webhook + sending domains. Fable-audited 2026-07-18 ×2 (review fixes 7b216ed + unconditional webhook sig verify). After merge: server deploy + env (`RESEND_FROM_MARKETING` subdomain split, `RESEND_WEBHOOK_SECRET` — webhook events DROPPED until set, `PUBLIC_API_BASE`). Open decisions filed: **#59 abuse gate (launch-blocker)**, #60 compliance follow-ups (service messages, resubscribe, CAN-SPAM address, >1000 batching, UX polish); list blobs → #45 cutover (commented) | **Fable** | PR open, CI pending |
