...
| # | Item | Owner | State |
|---|---|---|---|
| **0000** | **Claim-feed paging + lenient-read hardening.** Claimers paged (#112, merged). Pending-claims paged + one per-series write queue + strict reads on the write path (PR #117) — also fixes reject clearing a slot it no longer owns, and the blank-claims-page fallback in approve/reject. **Then, in order:** #113 (`claimTicket`'s slot scan treats an unreadable claims page as empty — wipes claims + double-assigns editions; hot money path, own review), #114 (same lenient-read hole in `claimers-feed.ts`, + extract the shared page applier), #115 (approve/reject rewrite claims pages outside `queueSeriesClaim`), #116 (agent rail bypasses that queue — latent, crypto rail off). **Not deployed** — rides with the #110/#111/#112 batch. | **Fable** (money path) | #117 open, CI green pending |
