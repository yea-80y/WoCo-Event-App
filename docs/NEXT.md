...
Last updated: 2026-08-25 (deploy-state claims removed — see below).

> **THIS FILE DOES NOT RECORD DEPLOY STATE, deliberately (#187).** It used to, and the
> claims were written at planning time and never updated when the owner actually deployed
> — so three separate rows went on saying a frontend deploy was outstanding long after it
> had happened. On 2026-08-06 that nearly caused a duplicate production deploy: the file
> was read, believed, and the owner had to correct it. A doc cannot keep a fact true that
> it has no way to observe, so it no longer states one. **To find out what is live:**
>
> ```
> stat -c %y .swarm/swarm-feed-info.json          # when the frontend feed was last published
> python3 -m json.tool < .swarm/swarm-feed-info.json   # latestSiteReference + manifestRef
> curl -s https://events-api.woco-net.com/api/health   # what the server is answering
> ```
>
> Then prove it by CONTENT, not timestamp: fetch the reference and grep for a string
> unique to the change you are asking about. `.swarm/` is gitignored, so this only works
> on the machine that deploys. Historical "DEPLOYED on <date>" notes below are kept as a
> record of what happened; they are not a claim about what is live now.
...
| 7 | Gate B (POD certificate rail) implementation | Opus builds, Fable signs off | **Slices 1+2 MERGED (`38bfb18` PR #339, `ebc8082` PR #341, server deployed 2026-08-20). Slice 3 on `feat/pod-cert-issue`:** the issuer write path. Design pass corrected the premise — a lost log write is a LOST CERTIFICATE, not an audit gap, because until holder import exists a certificate lives nowhere else. So writes are verified and awaited, and a run stops at the first unverified write. `woco.pod-cert-log.v1` batches certificates per SOC version (~60 writes for 500 holders instead of 500) and is size-bounded so it can never reach the `_woco_mc` paging path; that exposed an unbounded `evidence` field in the frozen cert schema, now refused at signing. Chain-free minting ships (single-leaf real tree, `totalSupply` = declared cap, audit-enforced not gate-enforced), plus `certLogOwner` — without which a badge is mintable and its log unfindable. Caught before shipping: `knownVersion` behind a `Parameters<>` cast that `writeContentFeedVerified` did not accept, which would have silently reverted every page write to probing. **NOT built, so the write path is UNEXERCISED against a real gateway:** the creator issuance surface and the first real run. Residues filed: **#343** (nothing publishes holder X25519 keys, so `encPubKey` stays empty). Earlier slice detail:  `PodGate` is now a discriminated union (`ChainPodGate | CertPodGate`) — absent `holdingSource` means CHAIN (historically true and safe by direction), present-but-unrecognised REFUSES. Cert gates store `swarmManifestRef`, never `issuerPubkey` (a stored key can't go stale but can be BORN WRONG, which would manufacture exactly the object slice 1's carried finding warns about); the issuer is re-derived per enforcement through `podCertHoldingFromManifest`, which closes that finding by not taking a key at all. Write boundary refuses duplicate badges per group (a real latent bug once two sources exist — enforcement reads one holding per manifestRef), unknown sources, `minCount > 1`, `maxSlotExclusive`, and unresolvable manifests. Challenge round trip is SPECIFIED in SWARM_SOCIAL_PLAN and built with the holder slice. Editor deliberately does not offer the cert source yet. Pre-existing defect found and filed: **#340**. ~~Slice 1 detail~~ (`feat/pod-cert-rail`): the crypto core in `packages/shared/src/pod-cert/` — `woco.pod-cert.v1` + `woco.pod-cert-challenge.v1`, the issuer's banded log topics, and the pure door (`resolvePodCertIssuer` → `checkPodCertPresentation` → `podCertHolding`). No I/O, no chain, nothing written to an address yet; `evaluatePodGate` is untouched and takes the resulting `PodHolding` as-is. 35 new tests, frozen vectors; shared 242 green, server builds. Five decisions the design did not settle are recorded in SWARM_SOCIAL_PLAN's BUILD RECORD — four of them frozen. **Fable sign-off 2026-08-20: MERGE** (frozen vectors independently recomputed from raw primitives; canonicaliser/validator field sets congruent). Two of its three findings applied here; the third is carried: the door takes a bare `issuerPubkey`, and `PodDirectoryEntry.issuer` is an unverified mirror in the same package that type-checks as it — brand the resolve return type or make the door take the `SignedManifestV1`, when slice 2 writes the first caller. **Remaining:** the issuer write path + `PodGate.holdingSource` opt-in (with a chain-free write-boundary check replacing `verifyPodGateBinding`, and a REFUSAL of `minCount > 1` / `maxSlotExclusive` on certificate gates, which fail closed today but silently); then passport import + challenge UX; then the two proofs the plan demands (two-browser e2e with the server BLOCKED, and the passkey cost of a challenge signature). |
| 8 | Lever 3 — checkpoint POSITION inside a band, not just the band | — | **Quantified, deliberately NOT done.** Cold read is now a function of position-in-band only (misses are flat): 1.4s just after a rollover, 3.4s mid-band, 7.3s at 62/64. A coarse checkpoint would flatten that to ~1.4s, worth ~6s at the worst point. Not recommended for the Rita pilot — those riders stay inside band 0 and never roll over, so 7.3s is their ceiling too. A PER-LAP head pointer stays rejected (circular: it moves a write per lap onto the index feed, whose own resolution becomes the O(laps) scan). |
| 9 | Band-opening tap costs ~10.7s, once per 64 laps | — | Now the worst single moment in the rail. Two ~3s API calls inside it are the `thorough` index reads that deliberately do not trust the gateway gate, because that read feeds a read-modify-write — a correctness trade, NOT to be "optimised" away. The rest is genuine rollover work. Not a regression (comparable taps were 9.5s pre-fix). |
...
| 0 | **Marketing audience** — ✅ **PR #58 merged `3b1661e`, server DEPLOYED 2026-07-18** (Fable-audited ×2; /u + webhook smoke-tested live: bad token 404s, unsigned webhook ack-and-dropped). Remaining, in order: (a) **user**: Resend dashboard — add webhook endpoint `https://events-api.woco-net.com/api/resend/webhook` (email.bounced + email.complained) and set `RESEND_WEBHOOK_SECRET` in `.env` (+ STEP 2 deploy) — **bounce/complaint suppression is INERT until then**; (b) **user**: split `RESEND_FROM_MARKETING` onto its own subdomain BEFORE first cold-list send; (c) **user**: browser-test the Audience flow against whatever is live (see the header for how to check); (d) ~~#59 abuse gate~~ ✅ decided+built 2026-07-18 (charges_enabled on broadcast+domain-create, StripeVerifyGate in Audience UI); #60 follow-ups still open; list blobs → #45 cutover (commented) | **user / Fable** | Server live; env + #60 pending |
| 1 | ~~**Frontend deploy** (`npm run deploy`)~~ — this row asserted a pending deploy and had no way to stop asserting it. The standing rule is unchanged and worth keeping: **deploy the server before the frontend**, and restart `bee-proxy` after the frontend (#374). | **user** | Whether a deploy is outstanding is not recorded here — see the header. |
| 2 | **Browser-verify #52** — free-hosting gate + quota + batch routing, against the live frontend | **user** | Watch `docker compose logs -f server \| grep -E "batch-router\|storage-ledger"`; ledger = `docker compose exec server cat .data/storage-ledger.json`. |
