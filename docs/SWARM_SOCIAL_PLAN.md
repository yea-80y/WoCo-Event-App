...
- **P3: server drops out of the write path.** Browser/light clients upload to the same
  feeds themselves and announce over transport (app-level libp2p, or Waku — LightPush/
  Filter/Store fit browser clients; decide when real). Feeds/keys/addresses unchanged →
  nothing migrates. Personal views (own timeline) compile client-side with no indexer.
