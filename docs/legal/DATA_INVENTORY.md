...
| `event-attendees.json` | **Yes** (pseudonymous) | Event id → HMAC-SHA256 email hashes of ticket-holders — `attendee-index.ts`. Written at Stripe fulfilment from the verified purchase email, for every buyer who supplied one, INDEPENDENTLY of any marketing choice: it records the contract relationship (this person bought a ticket), not a consent. Lawful basis Art. 6(1)(b) — it is what proves a broadcast recipient is an attendee, so without it an organiser cannot tell ticket-holders their event was cancelled. Never plaintext. Not erasable via the marketing path: erasure here is governed by the ticket record, not the marketing consent record |
...
2. `broadcast-chunks/*.bin` — the recipients of an in-flight broadcast, **encrypted under a
   per-job key held only in process memory**, deleted per chunk as it drains and destroyed by a
   hard bound regardless (7 days at most, for a paced first send). Not readable at rest, not
   readable once the job ends, and not recoverable after a restart.
...
instructed, for the same purpose, for the life of the send — hours, or days for a paced first send
— rather than indefinitely, under a key that does not survive the job. The distinction is the one the objection actually turned on. It is
...
| `woco_ctx_job` | Broadcast job id, a random UUID | Attributes a bounce or complaint that arrives after acceptance to the send it came from, so a sender's paced sending can pause on it (#619). Random, derived from no person |
| `woco_ctx_batch` | `p` for contacts already proven deliverable, or `u<n>` for the n-th batch of new contacts | Lets a bounce or complaint be counted against the right denominator — the new-contacts check judges new contacts on their own. Carries no identifier |
...
— establishing that a paid-for ticket was not delivered, and (for the job and batch tags)
protecting the shared sending reputation. Tags are read back on hard bounces and, since #619,
on complaints too.
