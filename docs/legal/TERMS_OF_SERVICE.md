# Terms of Service

**Last updated:** [DATE OF PUBLICATION]
**Version:** 1.0

> **⚠️ PRE-LAUNCH DRAFT — NOT YET IN FORCE.** Complete the `[PLACEHOLDERS]` and have a UK solicitor
> review before publication. Consumer terms are subject to the Consumer Rights Act 2015 fairness
> test; an unfair term is unenforceable, so the wording here matters commercially, not just legally.

These terms cover **using WoCo as an attendee** — browsing events, buying tickets, holding tickets.
If you run events, the [Organiser Terms](./ORGANISER_TERMS.md) also apply.

---

## 1. Who you are contracting with

WoCo is operated by [COMPANY LEGAL NAME], registered in England and Wales, company number [NUMBER],
registered office [ADDRESS] ("WoCo", "we", "us").

**Important — we do not sell you the ticket.** When you buy a ticket, your contract for the event is
with the **event organiser**, not with us. We provide the platform they use to sell it, and we act as
their disclosed agent in taking your order. The organiser's identity is shown at checkout.

Payment is taken by the organiser on their own payment account. Their name — not ours — appears on
your card statement.

**What this means for you:** questions about the event, refunds, cancellations, entry conditions and
access requirements go to the **organiser**. Questions about the platform, your account, or your
ticket not arriving go to **us**. We will always help you reach the organiser, and if the platform
caused the problem we will fix it.

---

## 2. Using WoCo

You must be **16 or over** to create an account.

You agree not to: break the law; buy tickets to resell them commercially without the organiser's
permission; use bots or automation to obtain tickets; interfere with the platform's security or
availability; attempt to access data belonging to others; or use WoCo to send spam or harass anyone.

We may suspend or close an account that breaches these terms. Where we do, we will tell you why
unless we are legally prevented from doing so, and you can appeal to [SUPPORT EMAIL].

### Your account and your keys

WoCo supports several sign-in methods, including passkeys and crypto wallets. Some of these mean
**you hold the keys, and we cannot recover them for you.**

Where you control the keys:

- **We cannot reset your access.** There is no password reset for a self-custodied wallet.
- **Losing your key may mean losing access to your tickets and any on-chain records.**
- We provide recovery tools where we can, and we strongly encourage you to use them at signup.

This is a genuine trade-off of self-custody, not a limitation we have chosen to impose. Please set up
recovery when prompted.

---

## 3. Tickets

**Your ticket comes from the organiser.** We issue it on their behalf, on their instructions, with
their terms.

- A ticket is a **licence to attend**, granted by the organiser and subject to their conditions.
- The organiser sets the price, capacity, entry requirements and refund policy.
- The organiser may cancel, reschedule or change the event. Your rights in that case are against them.
- We may need to reissue or correct a ticket to fix a technical problem.

**Your ticket record is stored on decentralised infrastructure and is designed to last.** We do not
delete tickets by default — they are your record of what you attended. See the
[Privacy Policy](./PRIVACY_POLICY.md) section 8 for how that works and what its limits are.

---

## 4. Payment and refunds

Prices are set by the organiser and shown before you pay, including any fees.

**Refunds are the organiser's responsibility.** Their refund policy applies. If an event is cancelled,
the organiser is responsible for refunding you.

**We are not obliged to refund you from our own funds** for an organiser's cancellation or failure —
we are not party to the sale. If an organiser becomes insolvent or refuses a refund you are entitled
to, we will: give you everything we hold that evidences your purchase; support a chargeback with your
card issuer; and pass on any funds we are holding that are due to you. Your card chargeback rights
are usually your strongest remedy, and we will not obstruct them.

**What our payout timing does and does not do for you.** We generally do not release an organiser's
takings to them until after their event has happened. That makes it more likely funds are still
available if a refund becomes due, and it is deliberate. **It is not a guarantee, and it is not
escrow or any form of protected client account.** Specifically: the money sits in the organiser's own
account with our payment provider, not ours; the provider's rules do not allow funds to be held
indefinitely, so tickets bought well in advance are paid to the organiser before the event; and an
organiser may be able to withdraw their balance themselves. **Do not treat this as insurance against
an organiser failing.** Your chargeback rights remain your strongest remedy.

**Your statutory rights are unaffected.** Nothing here overrides the Consumer Rights Act 2015.

**Cancellation rights.** Under the Consumer Contracts Regulations 2013, the 14-day cooling-off period
generally **does not apply** to tickets for events on a specified date. That is a statutory exception,
not a term we have imposed. The organiser may still offer refunds voluntarily.

**Platform fees.** Where we charge a fee, it is shown before you pay. Our fees are non-refundable
once the ticket has issued, except where we are at fault or the law requires otherwise.

---

## 5. What we provide, and what we don't

We provide the platform "as is" and aim to keep it available, but we do not guarantee uninterrupted
service. We may change or discontinue features, and will give reasonable notice of material changes
where we can.

**We do not vet or endorse events or organisers.** We are a platform, not a promoter. We do not
guarantee an event will happen, be as described, or be any good.

**We do not control the decentralised networks we use.** Swarm and the blockchains we interact with
are public infrastructure operated by third parties. Their availability is outside our control.

---

## 6. Our liability

Nothing in these terms limits our liability for death or personal injury caused by our negligence,
for fraud, or for anything else that cannot lawfully be limited.

Subject to that:

- We are **not liable for the event itself** — its cancellation, quality, safety or conduct. That is
  the organiser's responsibility.
- We are **not liable for losses caused by you losing your own keys**, where you control them.
- We are **not liable for failures of public decentralised networks** outside our control.
- We are not liable for indirect or consequential loss, or loss of profit, revenue or opportunity.

**Where we are at fault**, our total liability to you is limited to the greater of the fees we
received in connection with the affected transaction, or £[LIABILITY CAP].

If you are a consumer, this section does not affect your statutory rights, and you may bring claims
against the organiser directly.

---

## 7. Your content

You keep ownership of anything you upload (profile details, images, messages). You grant us a licence
to host, store and display it as needed to run the service.

**Content published to decentralised storage may be permanent.** If you publish a profile, event or
site, it may not be possible to delete it fully from the network. Please do not publish anything you
would not be comfortable being public indefinitely.

You must not upload anything unlawful, infringing, or that you do not have the right to share.

---

## 8. Changes to these terms

We may update these terms. Material changes will be notified by email or in-app notice before they
take effect. Continuing to use WoCo after that means you accept the change. If you do not accept, you
may close your account; tickets you already hold remain valid.

---

## 9. Referral scheme

If you share a referral link and an organiser signs up through it, WoCo pays you **40% of its
1.5% platform fee** on ticket sales that organiser makes.

**This is a launch rate.** WoCo may vary or withdraw it at any time on reasonable notice, and may
set a different rate for different referrers or different periods. Any change applies from the
date of notice and **does not affect earnings you have already accrued**.

Earnings build up per sale and are released after the event, at the same time the organiser is
paid. WoCo intends to move referral payouts to automated stablecoin settlement once the relevant
contracts have been independently audited; until then they are paid manually.

Referral earnings are payment for introducing a customer. **They are not an investment and give
you no interest in WoCo.**

We may withhold or reverse referral earnings arising from self-referral, fraudulent or refunded
sales, or any breach of these terms.

---

## 10. Law and disputes

These terms are governed by the law of **England and Wales**, and the courts of England and Wales
have jurisdiction.

If you live elsewhere in the UK, you may bring proceedings in your local courts. If you are a
consumer in the EU, mandatory protections of your home country still apply.

**Please contact us first** at [SUPPORT EMAIL] — most things are resolved quickly that way.

---

## 11. Contact

[COMPANY LEGAL NAME]
[REGISTERED OFFICE ADDRESS]
[SUPPORT EMAIL]
