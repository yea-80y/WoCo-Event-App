 ...
 * `checkObjectGate`, but nothing in the codebase constructs one, so `resolveHolding`
...
import type { ObjectDirectoryEntry } from "@woco/shared";
import {
  partitionGateable,
  buildChainGates,
  notGateableLabel,
...
function entry(over: Partial<ObjectDirectoryEntry> = {}): ObjectDirectoryEntry {
  return {
    ...
    ...over,
  } as ObjectDirectoryEntry;
}

const chainObject = entry({ eventId: `0x${"aa".repeat(32)}`, chainId: 421614 });
const certObject = entry({
  manifestRef: `0x${"22".repeat(32)}`,
  name: "Century Rider",
  certLogOwner: "0x3333333333333333333333333333333333333333",
  swarmManifestRef: "ab".repeat(32),
});
const unregistered = entry({ manifestRef: `0x${"33".repeat(32)}`, name: "Half-minted" });

// ---------------------------------------------------------------------------
// What can gate
// ---------------------------------------------------------------------------

test("a registered chain object is gateable", () => {
  const { gateable, blocked } = partitionGateable([chainObject]);
  assert.equal(gateable.length, 1);
  assert.equal(blocked.length, 0);
});

test("a certificate badge is BLOCKED, not gateable — no presentation path exists", () => {
  const { gateable, blocked } = partitionGateable([certObject]);
...
test("a certificate badge is SHOWN rather than filtered away", () => {
  ...
  const { blocked } = partitionGateable([chainObject, certObject]);
  assert.deepEqual(blocked.map((b) => b.objectEntry.name), ["Century Rider"]);
});
...
test("blockedness does not depend on how many certificates were issued", () => {
  ...
  const issued = { ...certObject, issuedCount: 40 };
  assert.equal(partitionGateable([issued]).blocked[0]!.reason, "cert-not-live");
});

test("an unregistered chain object is blocked for a DIFFERENT, honest reason", () => {
...
test("a cert badge missing chain coordinates is not mistaken for unregistered", () => {
  ...
  // never coming.
  assert.equal(partitionGateable([certObject]).blocked[0]!.reason, "cert-not-live");
});

test("every object lands in exactly one bucket — none is lost", () => {
  const all = [chainObject, certObject, unregistered];
...
test("CHAIN ARM UNCHANGED: the emitted gate is field-for-field what it always was", () => {
  const gates = buildChainGates([chainObject.manifestRef], [chainObject]);
  assert.equal(gates.length, 1);
  assert.deepEqual(gates[0], {
    manifestRef: chainObject.manifestRef,
    onChainEventId: chainObject.eventId,
    chainId: chainObject.chainId,
    objectName: chainObject.name,
  });
  assert.equal(
...
test("a certificate badge cannot be emitted even if its ref is selected", () => {
  ...
  const gates = buildChainGates([certObject.manifestRef], partitionGateable([certObject]).gateable);
...
test("a stale ref naming no known object drops out rather than emitting a broken gate", () => {
  const gates = buildChainGates([`0x${"ff".repeat(32)}`], [chainObject]);
  assert.deepEqual(gates, []);
});

test("selection order is preserved for multi-object gates", () => {
  const second = entry({
  ...
  const gates = buildChainGates([second.manifestRef, chainObject.manifestRef], [chainObject, second]);
  assert.deepEqual(gates.map((g) => g.objectName), ["Second", "Founding Member"]);
});
