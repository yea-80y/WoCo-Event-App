...
<div class="pod-manager">
  ...
  {#if phase === "ready" && pods.length > 0}
    <div class="filter-row">
      <div class="filters" role="tablist" aria-label="Filter collectibles by category">
        <button
          class="fchip"
  ...
  {:else if phase === "unauth"}
    <div class="empty-state">
      ...
      <p>Connect your account to manage your collectibles.</p>
    </div>
  {:else if phase === "error"}
  ...
  {:else if pods.length === 0}
    <div class="empty-state">
      ...
      <span class="kicker">No collectibles yet</span>
      <p>
        Collectibles are your ownable assets — event tickets, loyalty badges, limited
        drops. Publish an event and its tickets appear here automatically, or
        create a badge to reward your community.
      </p>
      <button class="btn btn--primary" onclick={onCreate}>Create your first collectible</button>
    </div>
  {:else if filtered.length === 0}
    <div class="empty-state">
      <span class="kicker">Nothing in this category</span>
      <p>No collectibles match this filter yet.</p>
    </div>
  {:else}
    <div class="grid">
