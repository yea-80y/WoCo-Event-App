<script lang="ts">
  ...
  async function load() {
    ...
    try {
      ...
      error = e instanceof Error ? e.message : "Failed to load collectibles";
      phase = "error";
    }
  }
...
<div class="gate">
  <label class="gate-toggle">
    ...
    <span class="gate-label">Require holding a collectible</span>
    <span class="gate-hint">Buyers must be signed in with a wallet that holds the chosen collectible(s) on-chain. They can still pay any way you accept — card or crypto.</span>
  </label>

  {#if enabled}
    <div class="gate-body">
      {#if phase === "loading"}
        <p class="gate-msg">Loading your collectibles…</p>
      {:else if phase === "error"}
      ...
      {:else if gateable.length === 0 && blocked.length === 0}
        <p class="gate-msg">
          No collectibles yet. Create one in the
          <a href="/creator/pods">Collectibles</a> manager first.
        </p>
      {:else if gateable.length === 0}
        <p class="gate-msg">
          None of your collectibles can gate yet.
        </p>
        {@render blockedList()}
      {:else}
        <div class="gate-grid">
        ...
        {:else if windowKind === "firstN"}
          <div class="firstn-row">
            ...
            <span class="firstn-hint">
              tickets are for collectible holders only; after {Math.max(1, Math.floor(winFirstN) || 1)} are
