<script lang="ts">
  /**
   * Shared catalog editor — categories + products CRUD for one shop. Used by both
   * the website-builder ShopTab and the standalone Shop editor so the editing UX
   * lives in one place. Owns its own product list (loads by shopId); category
   * edits go through updateShop and bubble up via onShopUpdate so the parent's
   * shop stays in sync.
   */
  import type {
    Shop,
  ...
  interface Props {
    shop: Shop;
    /** Bubble category/shop changes so the parent's shop reference stays current. */
  ...
  onMount(async () => {
    try {
      products = await getProducts(shop.shopId);
    } catch {
      // non-fatal: empty list, the merchant can still add products
    } finally {
      loading = false;
    }
  });
  ...
  async function addCategory() {
    ...
    catError = "";
    const cat: ProductCategory = { id: uid(), label: catInput.trim(), sortIndex: shop.categories.length };
    try {
      const updated = await updateShop(shop.shopId, { categories: [...shop.categories, cat] });
      onShopUpdate(updated);
      catInput = "";
    } catch (e) {
      catError = e instanceof Error ? e.message : "Couldn't add category";
    }
  }
  ...
  async function removeCategory(id: string) {
    ...
    try {
      ...
      catError = e instanceof Error ? e.message : "Couldn't remove category";
    }
  }
  ...
  async function handleDeleteProduct(p: Product) {
    try {
      ...
      // leave the list as-is; a failed delete just means it's still there
    }
  }
...
<div class="section-block">
  ...
  {#if shop.categories.length > 0}
    <ul class="cat-list">
      {#each [...shop.categories].sort((a, b) => a.sortIndex - b.sortIndex) as cat (cat.id)}
        <li>
  ...
  {#if catError}<div class="err-box mono">{catError}</div>{/if}
</div>
...
<div class="section-block">
  ...
  {#if productFormOpen}
    <div class="product-form">
      <div class="pf-head">
        ...
        <button class="close" onclick={() => { productFormOpen = false; }} aria-label="Close">
          <svg width="10" height="10" viewBox="0 0 10 10" fill="none" stroke="currentColor" stroke-width="2.25" stroke-linecap="square"><path d="M1 1l8 8M9 1L1 9" /></svg>
        </button>
      </div>
      <label class="field">
      ...
      <label class="field">
        ...
        <select class="input" bind:value={pCategory}>
          ...
          {#each shop.categories as cat (cat.id)}
  ...
  {#if loading}
    <p class="empty-hint">Loading products…</p>
...
<style>
  ...
  .count { font-family: var(--font-mono); font-size: 0.625rem; color: var(--text-dim); }
