<script lang="ts">
  ...
  async function createWocoListing() {
    ...
    listingOnWoco = true;
    wocoListError = null;
    // The server resolves an unlisted client-signed event from trusted sources
    // only: its event cache (primed at publish) or our platform-written creator
    // index — a client-supplied signer carrier is deliberately not accepted.
